...
async function login(page, credentials = {}) {
    const url = credentials.url || config.lms.url;
    const username = credentials.username || config.lms.username;
    const password = credentials.password || config.lms.password;

    if (!username || !password) {
        throw new Error('Username atau Password LMS belum disetel di .env (LMS_USERNAME, LMS_PASSWORD)');
    }
